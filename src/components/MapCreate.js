...
class MapCreate extends React.Component {
  ...
  componentDidUpdate() { //link를 쓰면 지도 창안에서 리스트를 다시 선택할때 render가 안되서 다시 리로드를 해줌. 정확한 원인은 모르겠음.
        ...
        <Navbar className = "navbar">
          ...
          <div className="navbar_find">
            ...
            <DropdownButton  id="dropdown-basic-button" title="선택">
                  {information.map((element) => // a 태그를 쓰게 될 경우 a 태그의 기본적인 속성때문에 새로 렌더링이 되기 때문에, Link를 써주어서 새로 렌더링되는 것을 막아주어야 한다.
                      <Dropdown.Item as={ Link } key={element.id} to={`/map/${element.id}`} onClick={this.refresh}>
                        {element.name}
                      </Dropdown.Item>
                  )}
                </DropdownButton>
...
export default withRouter(MapCreate);
